...
We evaluate \physattest{} on two datasets: (i)~the real SWaT dataset from iTrust SUTD~\cite{goh2017dataset} (51 sensors, 449{,}919 attack samples with 35 labelled attack periods, obtained under NDA), and (ii)~a synthetic SWaT testbed with 22 continuous sensors and 13 attack scenarios for ablation studies. All experiments run on a standard laptop (CPU only) using CVXPY, LangGraph, SciPy, and NumPy.

On the real SWaT dataset, the multi-domain observer (Layer~1) with Shewhart+CUSUM+Bias three-layer detection achieves precision 0.316 at FPR 0.0034 and accuracy 0.879, using 5 of 7 active detection states (pH and ORP are auto-excluded due to systematic model mismatch). The strongest single-attack result is Attack~12 (84.1\% recall, 100\% precision, 28s delay). The overall low recall (1.2\%) is expected: the Layer~1 observer covers only hydraulic and conductivity states; the remaining states require SINDy Layer~2 refinement (now wired in) and the anomaly transformer (Component~6).

\subsection{Detection Performance (Synthetic)}

Table~\ref{tab:detection_metrics} shows the precision-recall tradeoff across detection thresholds on synthetic data. The best F1 score of 0.79 is achieved at threshold $\tau = 2.5$, with precision 0.84 and recall 0.75.

\input{../physattest/experiments/output/tables/tab_detection_metrics}

\subsection{Per-Attack Breakdown}

\input{../physattest/experiments/output/tables/tab_per_attack}

\subsection{Component Ablation}

\input{../physattest/experiments/output/tables/tab_component_ablation}

\subsection{Comparison with Baselines}

\input{../physattest/experiments/output/tables/tab_comparison}

\subsection{Theorem Verification}

\input{../physattest/experiments/output/tables/tab_theorem_verification}
...
We presented \physattest{}, a 13-component defense framework for agentic IoT systems that grounds all security decisions in physical laws. The framework simultaneously addresses sensor spoofing, LLM agent hijacking, and server code tampering through five coordinated LangGraph agents protected by a CBF safety filter. Three formal theorems establish the system's security guarantees: a game-theoretic impossibility bound, an exponentially decaying fingerprint evasion bound, and Byzantine fault tolerance with up to two compromised agents. Evaluation on the real SWaT dataset demonstrates 87.9\% accuracy and 0.34\% FPR with the three-layer detector, while synthetic experiments achieve F1$=$0.79 with the CBF blocking all overflow attacks, running entirely on CPU in under 2~seconds.

Future work includes: (i)~completing SINDy Layer~2 integration to recover pH/ORP detection, (ii)~training the anomaly transformer on real SWaT residuals, (iii)~extending the Stackelberg game to multi-round repeated interactions, and (iv)~deploying on a physical testbed with real ADC noise fingerprints.
